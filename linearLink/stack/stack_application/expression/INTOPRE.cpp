...
string infix_to_prefix(const string & infix)
{
    ...
    // and input them into the prefix expression.
    while( !optrs.empty() )
    {
        prefix += optrs.top();
        optrs.pop();
    }
    //reverse the prefix expression to make it fit the standard.
...
int main()
{
    ...
    while(1)
    {
        ...
        if(infixExp == "q")
            break;
        prefixExp = infix_to_prefix(infixExp);
